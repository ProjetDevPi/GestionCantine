 ...
 * @author aymen
 */
public class Commande_AdminForm extends Form{
  EncodedImage enc;
   Form prev;
    public Commande_AdminForm(Form Back) {
 ...
 if (p.getId() == x.getId_plat_nom())
 {
        ...
        this.revalidate();
          ...
          this.getToolbar().addCommandToRightBar("stat", null, (evt) -> {
            new statCommande().createPieChartForm("Commande", new ServiceMenu().getstatcommandes());
            });
...
Image imgs2;
            ...
            f2.add(imgv2).add("nom plat : ").add(nom).add("Boisson :").add(c.getBoisson()).add("Desert :").add(c.getDessert()).add("Supplement :").add(c.getSupplement()).add("prix :").add(String.valueOf(c.getPrix())).add(Supprimer);
   Supprimer.addActionListener(ev
                    -> {
                new ServiceCommande().Deletecommande(q.getId());
                new Commande_AdminForm(prev).show();
            }
            );
